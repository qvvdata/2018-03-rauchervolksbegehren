...
teiler <- quantile(analysis_gem$Anteil,probs=einheiten)

analysis_gem$cut <- cut(analysis_gem$Anteil,
                        breaks=c(0,teiler),
                        labels=paste(seq(10,100,10),"%",sep=""),
                        include.lowest=TRUE)

analysis_gem$klasse <- analysis_gem$cut

analysis_gem <- analysis_gem %>%
  mutate(klasse = gsub('%', '', klasse))%>%
  numerize(vars = c("Name", "cut"))%>%
...
write.csv(analysis_gem, "interaktiv/karte/dist/analysis_gem.csv",fileEncoding='UTF-8',row.names = F)
...
wbez <- unterstuetzungen_8_3 %>%
  ...
  mutate(pct= Unterstützungen/Wahlberechtigte*100)%>%
  select(c(Name, pct))
write.csv(wbez, "output/datawrapper/wbez.csv")
...
corrdf2018 <- corrdf %>%
            subset(select=c(oevppct, spoepct, gruenepct, fpoepct, sonstigepct))
```
...
analysis_orf <- analysis_orf %>% left_join(data, by=c("gkz", "gkz")) %>%
    mutate(
...
analysis_orf_gem <- analysis_orf %>%
  ...
  select(gkz, `Anteil gesamt`, `Anteil Unterstützungsphase`, Unterstützer, Wahlberechtigte,Name, `türkis/blau-Anteil`, Steigerung)
write.csv2(analysis_orf_gem, "output/datawrapper/analysis_orf_gem.csv")
...
write.csv2(analysis_orf_gem, "output/datawrapper/analysis_fvb_gem_alle.csv")
