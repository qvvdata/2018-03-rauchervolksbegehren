...
#Analyse
analysis <- data %>% left_join(unterstuetzungen, by=c("gkz", "gkz")) %>%
    mutate(quantile = ntile(pct, 100))
...
analysis_gem <- analysis %>%
  ...
  select(gkz, unterstützungen, wb, pct, 'name.x', tkbl)%>%
  rename(Name = 'name.x', 
         Anteil = pct, 
         Wahlberechtigte = wb, 
...
write.xlsx(analysis_gem, "output/datawrapper/analysis_gem.xlsx")
